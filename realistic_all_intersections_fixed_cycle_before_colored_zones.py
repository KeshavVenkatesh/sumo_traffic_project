...
OD_ACTIVE_APPROACH_MIN_SAMPLES = 5
OD_ACTIVE_APPROACH_MAX_SUCCESSOR_SHARE = 0.46
OD_ACTIVE_APPROACH_MAX_LEFT_SHARE = 0.42
OD_ACTIVE_APPROACH_MAX_RIGHT_SHARE = 0.55
OD_ACTIVE_APPROACH_MAX_STRAIGHT_SHARE = 0.78
OD_ACTIVE_APPROACH_RELAX_FRACTION = 0.96
...
OD_DESTINATION_ZONE_BALANCE_POWER = 3.0
OD_DESTINATION_OD_PAIR_BALANCE_POWER = 1.4
OD_DESTINATION_ZONE_HOTSPOT_SHARE = 0.12
OD_DESTINATION_ZONE_HOTSPOT_MULTIPLIER = 0.03
OD_DESTINATION_ZONE_EDGE_WEIGHT_POWER = 0.35
OD_DESTINATION_ZONE_HISTOGRAM_PRINT_INTERVAL = 60.0

# GUI zone-grid overlay.  When GUI mode is enabled, the OD destination zones
# are drawn as translucent blue grid lines on top of the SUMO map.  These are
# visual debugging markers only; they do not affect routing or traffic control.
OD_ZONE_GRID_LINES_ENABLED = True
OD_ZONE_GRID_LINE_THICKNESS = 35.0
OD_ZONE_GRID_LINE_COLOR = (0, 110, 255, 210)
OD_ZONE_GRID_BORDER_COLOR = (0, 70, 255, 245)
OD_ZONE_GRID_LINE_LAYER = 260
...
def cleanup_active_od_destination_tracking(sim_state):
    ...
    Also updates a car's destination zone if route-repair logic changed its
    final route edge while it was active.
    """
    context = sim_state.get("od_context")
    if not context:
        return
    ...
    for veh_id in list(dest_by_vehicle):
        ...
        current_destination = current_route_destination_edge_for_vehicle(veh_id)
        if current_destination is None:
            continue

        if current_destination != dest_by_vehicle.get(veh_id):
            try:
                route = list(traci.vehicle.getRoute(veh_id))
            except traci.TraCIException:
                continue
            register_active_od_destination_vehicle(sim_state, veh_id, route, context)
...
def remove_od_zone_grid_lines():
    """Remove old OD-zone grid polygons from the current SUMO connection."""
...
def _add_od_zone_grid_rectangle(poly_id, x1, y1, x2, y2, color):
    ...
    try:
        traci.polygon.add(
            ...
            color,
            fill=True,
            polygonType="od_zone_grid_line",
            layer=OD_ZONE_GRID_LINE_LAYER,
            lineWidth=1,
        )
...
def draw_od_zone_grid_lines(context, args):
    ...
    The grid is based on the same bounds and grid size used by build_od_zones(),
    so the visible cells match the destination balancing zones.  Lines are drawn
    as thin filled rectangles because that is more reliable in SUMO-GUI than
    relying on open polyline rendering.
    """
    ...
    for i in range(grid_size + 1):
        ...
        if _add_od_zone_grid_rectangle(poly_id, x - half, min_y, x + half, max_y, color):
            drawn += 1
    ...
    for j in range(grid_size + 1):
        ...
        if _add_od_zone_grid_rectangle(poly_id, min_x, y - half, max_x, y + half, color):
            drawn += 1

    print()
    print("OD destination-zone GUI overlay:")
    print(f"  grid:       {grid_size} x {grid_size}")
    print(f"  grid lines: {drawn}")
...
SPAWN_DESTINATION_MARKERS_ENABLED = True
