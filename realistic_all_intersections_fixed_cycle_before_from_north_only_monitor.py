...
def _tm_print_turn_monitor(force=False):
    ...
    print(
        "  approach      n     left      straight   right     unknown   wrong_lane",
        flush=True,
    )
    ...
    for approach in ["from_north", "from_east", "from_south", "from_west", "unknown"]:
        ...
        n = c["total"]
        left = 100.0 * c["left"] / n
        straight = 100.0 * c["straight"] / n
        right = 100.0 * c["right"] / n
        unknown = 100.0 * c["unknown"] / n
        wrong = 100.0 * _TARGET_TURN_MONITOR_WRONG_LANE[approach]["total"] / n

        print(
            f"  {approach:<11} {n:5d} "
            f"{left:8.1f}% {straight:10.1f}% {right:8.1f}% "
            f"{unknown:9.1f}% {wrong:10.1f}%",
            flush=True,
        )
