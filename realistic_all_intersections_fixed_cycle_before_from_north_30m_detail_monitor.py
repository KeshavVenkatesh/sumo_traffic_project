...
def _turn_monitor_after_simulation_step():
    """Live 150m turn-intention snapshot.

    At each print interval, this scans every vehicle currently within
    TARGET_TURN_MONITOR_APPROACH_DISTANCE of the target intersection and prints
    whether its planned next movement is left, straight, or right.

    This is not cumulative. It is a live snapshot at that timestep.
    """
    global _TARGET_TURN_MONITOR_LAST_PRINT
    ...
    if now - _TARGET_TURN_MONITOR_LAST_PRINT < TARGET_TURN_MONITOR_PRINT_INTERVAL:
        return
    ...
    try:
        ...
        rows = []
        counts = _TurnMonitorDefaultDict(_TurnMonitorCounter)
        ...
        for veh_id in traci.vehicle.getIDList():
            ...
            if dist_to_intersection < 0.0:
                continue
            if dist_to_intersection > TARGET_TURN_MONITOR_APPROACH_DISTANCE:
                continue
            ...
            next_edge = _tm_planned_next_edge(veh_id, current_edge)
            ...
            if next_edge is None:
                ...
                wrong_lane = next_edge not in lane_to_out_edges.get(lane_id, set())

            approach = _tm_approach_direction_from_lane(lane_id)

            counts[approach]["total"] += 1
            counts[approach][movement] += 1
            if wrong_lane:
                counts[approach]["wrong_lane"] += 1
        ...
        print(
            f"\n[target-turn-live-150m] t={now:.1f}s "
            f"tls={TARGET_TURN_MONITOR_TLS} vehicles_within_150m={total}",
            flush=True,
        )
        ...
        print(
            "  approach      n   left_n  straight_n  right_n  unknown_n  wrong_lane_n",
            flush=True,
        )

        for approach in ["from_north", "from_east", "from_south", "from_west", "unknown"]:
            c = counts.get(approach)
            if not c or c["total"] <= 0:
                continue

            print(
                f"  {approach:<11} {c['total']:4d} "
                f"{c['left']:8d} {c['straight']:11d} {c['right']:8d} "
                f"{c['unknown']:10d} {c['wrong_lane']:13d}",
                flush=True,
            )

        if total == 0:
            print("  No vehicles currently within 150m of the target intersection.", flush=True)
            print("", flush=True)
            return
        ...
        print(
            "  approach      dist_m   veh_id                 planned     wrong_lane  speed  lane_id                         current_edge                  next_edge",
            flush=True,
        )

        for approach, dist, veh_id, movement, wrong_lane, lane_id, current_edge, next_edge, speed in rows:
            print(
                f"  {approach:<11} {dist:7.1f} "
                f"{veh_id:<22} {movement:<10} "
                f"{str(wrong_lane):<10} {speed:5.2f} "
                f"{lane_id:<31} {current_edge:<29} {next_edge}",
                flush=True,
            )
        ...
        print(f"[target-turn-live-150m] WARNING: monitor failed: {exc}", flush=True)
