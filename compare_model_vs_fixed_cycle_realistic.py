...
def print_comparison_table(rows: list[dict[str, Any]]) -> None:
    ...
    print('\n' + '=' * 92)
    print('MODEL VS NO-MODEL FIXED-CYCLE COMPARISON')
    print('=' * 92)
    print(f"{'metric':28s} {'fixed_cycle':>16s} {'model':>16s} {'model improvement':>20s}")
    print('-' * 92)
    for metric, direction in metrics:
        fixed_value = float(fixed.get(metric, 0.0) or 0.0)
        model_value = float(model.get(metric, 0.0) or 0.0)
        pct = improvement_percent(model_value, fixed_value, higher_is_better=(direction == 'higher'))
        print(f'{metric:28s} {fixed_value:16.3f} {model_value:16.3f} {pct:19.2f}%')
    print('=' * 92)
    print('Positive improvement means the model beat the fixed-cycle baseline for that metric.')
...
def compare_model_to_fixed_cycle(args: argparse.Namespace) -> None:
    ...
    for seed in seeds:
        ...
        if args.run_order in {'fixed_first', 'both'}:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Compare a trained MaskablePPO traffic-light model against a no-model fixed-cycle baseline in the realistic SUMO simulation.'
    )
    ...
    parser.add_argument('--run-order', choices=['fixed_first', 'model_first', 'both'], default='fixed_first')
