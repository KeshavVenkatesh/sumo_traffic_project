...
def run_all_model_episode(args: argparse.Namespace, scenario: TrafficScenario, seed: int) -> dict[str, float | int | str]:
    ...
    Anchorless version: it never constructs a single-target environment and
    never chooses a template/anchor intersection from the map.
    """
    ...
    try:
        ...
        # Smooth GUI: advance SUMO one step at a time and stagger controller
        # policy updates across the same 5-second decision window.
        decision_period_steps = max(1, int(round(sim.DECISION_INTERVAL / sim.STEP_LENGTH)))
        decision_steps = 1

        controllable_count = sum(1 for c in episode.controllers if not c.get('disabled'))
        print(
            f'[all-model seed {seed}] anchorless control of {controllable_count} traffic lights '
            f'(no target TLS, staggered over {decision_period_steps} sim steps)'
        )
...
def run_fixed_cycle_episode(args: argparse.Namespace, scenario: TrafficScenario, seed: int) -> dict[str, float | int | str]:
    ...
    try:
        ...
        print(f'[fixed seed {seed}] anchorless fixed-cycle control of {sum(1 for c in episode.controllers if not c.get("disabled"))} traffic lights')
