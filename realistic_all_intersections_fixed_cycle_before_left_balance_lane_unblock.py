...
# Intersection green-light unblock watchdog.
# The general unjustified-stop watchdog intentionally ignores vehicles close to
# intersections because ordinary red-light queues are valid.  However, after
# adding strict lane preparation, some vehicles can remain stopped at a signal
# even after their route-compatible lane has a green and the downstream path is
# clear.  This watchdog is narrower: it only acts on stopped lead vehicles near
# junctions, first tries to repair lane/route mismatches, then clears stale
# keep-clear/speed/lane locks so SUMO can move the vehicle normally.
INTERSECTION_STUCK_UNBLOCK_ENABLED = True
INTERSECTION_STUCK_UNBLOCK_CHECK_INTERVAL = 1.0
INTERSECTION_STUCK_UNBLOCK_SPEED = 0.25
INTERSECTION_STUCK_UNBLOCK_WAIT_TIME = 4.0
INTERSECTION_STUCK_UNBLOCK_DISTANCE = 190.0
INTERSECTION_STUCK_UNBLOCK_LEADER_DISTANCE = 18.0
INTERSECTION_STUCK_UNBLOCK_COOLDOWN = 5.0
INTERSECTION_STUCK_TRACKING = {}
INTERSECTION_STUCK_LAST_ACTION = {}
...
def cleanup_route_lane_commitment_locks(active_ids):
    ...
    for veh_id in list(ROUTE_LANE_COMMITTED_VEHICLES):
        if veh_id not in active_ids:
            ...
            INTERSECTION_STUCK_TRACKING.pop(veh_id, None)
            INTERSECTION_STUCK_LAST_ACTION.pop(veh_id, None)
...
def set_route_lane_commitment_lock(veh_id, current_edge):
    """Mark the vehicle as route-lane committed without permanently boxing it in.

    The lane-preference helper still nudges the car toward a route-compatible
    lane, but this should not disable SUMO's ability to recover from a blocked
    lane.  The final no-lane-change zone still prevents last-second weaving.
    """
    try:
        traci.vehicle.setLaneChangeMode(veh_id, TRAFFIC_LIGHT_NORMAL_LANE_CHANGE_MODE)
...
def fresh_lane_links(lane_id):
    """Uncached lane links, used when current traffic-light state matters."""
    try:
        try:
            return tuple(traci.lane.getLinks(lane_id, extended=True))
        except TypeError:
...
def lane_signal_allows_next_edge(lane_id, next_edge):
    """Return True when the current lane's link to next_edge is currently passable.

    For traffic-light links, use the fresh link-state field rather than the
    cached lane-link helper.  For uncontrolled links, SUMO usually reports a
    non-red/non-yellow state or no dynamic state; treat those as passable.
    """
    saw_matching_link = False
    saw_signal_state = False

    for link in fresh_lane_links(lane_id):
        if not link:
            continue
        ...
        saw_matching_link = True

        state = ""
        if len(link) > 5 and link[5] is not None:
            state = str(link[5])

        if not state:
            return True

        saw_signal_state = True

        # G/g are protected/permissive green. M/m/O/o are non-red priority/open
        # states seen on some SUMO links. Unknown non-red/non-yellow states are
        # treated as passable so this watchdog does not depend on a single SUMO
        # version's exact link-state alphabet.
        if state in ("G", "g", "M", "m", "O", "o"):
            return True
        if state not in ("r", "R", "y", "Y", "s", "S"):
            return True

    if not saw_matching_link:
        return False

    # Matching link exists but all current signal states are red/yellow/stop.
    return not saw_signal_state


def cleanup_intersection_stuck_tracking(active_ids):
    active_ids = set(active_ids)
    for veh_id in list(INTERSECTION_STUCK_TRACKING):
        if veh_id not in active_ids:
            INTERSECTION_STUCK_TRACKING.pop(veh_id, None)
    for veh_id in list(INTERSECTION_STUCK_LAST_ACTION):
        if veh_id not in active_ids:
            INTERSECTION_STUCK_LAST_ACTION.pop(veh_id, None)


def try_unblock_stuck_intersection_vehicle(
    veh_id,
    raw_graph,
    edge_metadata,
    core_edges,
    ...
    """Emergency precaution for lead vehicles stopped at a green/clear approach.

    This does not force cars through red lights or into blocked exits.  It acts
    only on stopped lead vehicles near a junction.  If the current lane cannot
    legally reach the planned next edge, it calls the existing lane/route rescue.
    If the lane can reach the route and the signal/downstream path is clear, it
    clears stale keep-clear/lane/speed overrides and lets SUMO move the car.
    """
    if is_ambulance(veh_id):
        return False

    if not INTERSECTION_STUCK_UNBLOCK_ENABLED:
        return False
    ...
    approach_distance = max(
        INTERSECTION_STUCK_UNBLOCK_DISTANCE,
        traffic_light_no_lane_change_distance_for_lane(lane_id) + 40.0,
    )

    # Only handle vehicles close enough to be part of an intersection approach.
    if distance_to_end > approach_distance:
        INTERSECTION_STUCK_TRACKING.pop(veh_id, None)
        return False

    if speed > INTERSECTION_STUCK_UNBLOCK_SPEED:
        INTERSECTION_STUCK_TRACKING.pop(veh_id, None)
        return False

    # Do not push ordinary queued cars.  Only the lead car with no close leader
    # should be considered for emergency unblock.
    if vehicle_has_close_leader(veh_id, INTERSECTION_STUCK_UNBLOCK_LEADER_DISTANCE):
        INTERSECTION_STUCK_TRACKING.pop(veh_id, None)
        return False

    first_seen = INTERSECTION_STUCK_TRACKING.setdefault(veh_id, now)
    if waiting_time < INTERSECTION_STUCK_UNBLOCK_WAIT_TIME and now - first_seen < INTERSECTION_STUCK_UNBLOCK_WAIT_TIME:
        return False

    if now - INTERSECTION_STUCK_LAST_ACTION.get(veh_id, -1e9) < INTERSECTION_STUCK_UNBLOCK_COOLDOWN:
        return False

    next_edge = planned_next_edge_from_route(veh_id, current_edge)
    if next_edge is None or next_edge.startswith(":"):
        # A route ending exactly here can be legitimate.  Otherwise, reuse the
        # existing route-repair logic rather than letting the car block the node.
        if vehicle_route_end_is_near(veh_id, current_edge, distance_to_end):
            return False
        repaired = repair_unjustified_stop_route(
            veh_id=veh_id,
        ...
        if repaired:
            force_release_keep_clear_vehicle(veh_id)
            INTERSECTION_STUCK_LAST_ACTION[veh_id] = now
            INTERSECTION_STUCK_TRACKING.pop(veh_id, None)
            return True
        return False

    # If the lane cannot make the planned movement, do not wait for the car to
    # clog the stop line.  Try a lane change if feasible; if it is too late,
    # the existing rescue rewrites the route to a reachable outgoing edge.
    if not lane_has_connection_to_edge(lane_id, next_edge):
        recovered = rescue_vehicle_from_unconnected_lane(
            veh_id=veh_id,
            raw_graph=raw_graph,
            edge_metadata=edge_metadata,
            core_edges=core_edges,
    ...
    # If the route-compatible link is red/yellow, leave the car alone.  When it
    # becomes green, this same watchdog will clear stale holds if needed.
    signal_allows = lane_signal_allows_next_edge(lane_id, next_edge)

    # Do not force a vehicle into a downstream jam or occupied internal path.
    if not next_edge_has_exit_space(next_edge):
        return False
    if not internal_junction_path_is_clear(lane_id, next_edge):
        return False

    # Clear stale controls that can leave a car stopped even when the light and
    # exit are both usable.  The forced keep-clear release has a cooldown so our
    # custom gate cannot immediately re-hold the same car on the next step.
    force_release_keep_clear_vehicle(veh_id)
    safe_vehicle_set_speed(veh_id, -1)
    try:
        traci.vehicle.setLaneChangeMode(veh_id, TRAFFIC_LIGHT_NORMAL_LANE_CHANGE_MODE)
    ...
    # Only give a small movement nudge when the lane link is actually green/open.
    # If the signal is red, setSpeed(-1) is enough; SUMO will keep the car stopped
...
def apply_intersection_stuck_unblock_to_all_vehicles(
    raw_graph,
    edge_metadata,
    core_edges,
    turn_index,
    ...
    active_ids = list(traci.vehicle.getIDList())
    cleanup_intersection_stuck_tracking(active_ids)

    fixed = 0
    for veh_id in active_ids:
        if try_unblock_stuck_intersection_vehicle(
            veh_id=veh_id,
            raw_graph=raw_graph,
            edge_metadata=edge_metadata,
            core_edges=core_edges,
...
def apply_turn_lane_preference_to_vehicle(veh_id):
    ...
    if no_change_buffer > 0.0 and distance_to_end <= no_change_buffer + required_distance:
        # Too late for a safe lane change.  Leave the vehicle alone for now;
        # the intersection-stuck unblock watchdog will reroute it if it becomes
        # a stopped lead vehicle instead of letting it clog the approach.
        release_route_lane_commitment(veh_id)
        return False
...
def run_simulation_steps(
    ...
    for _ in range(num_steps):
        ...
        # Emergency green/clear-approach unblock.  This specifically handles
        # lead vehicles that remain stopped at an intersection after lane/route
        # prep, even though their lane can legally move and the downstream path
        # is clear.  It also reroutes them if the current lane cannot reach the
        # planned next edge.
        if sim_time >= sim_state.get("next_intersection_stuck_unblock_time", 0.0):
            recovered_total += apply_intersection_stuck_unblock_to_all_vehicles(
                raw_graph=raw_graph,
                edge_metadata=edge_metadata,
                core_edges=core_edges,
                turn_index=turn_index,
            ...
            sim_state["next_intersection_stuck_unblock_time"] = sim_time + INTERSECTION_STUCK_UNBLOCK_CHECK_INTERVAL
...
def run_simulation(args):
    ...
    try:
        ...
        sim_state = {
            ...
            "next_intersection_stuck_unblock_time": 0.0,
