...
TARGET_TURN_MONITOR_APPROACH_DISTANCE = 180.0
...
def _turn_monitor_after_simulation_step():
    try:
        ...
        for veh_id in traci.vehicle.getIDList():
            if veh_id in _TARGET_TURN_MONITOR_SEEN:
                continue
            ...
            _TARGET_TURN_MONITOR_SEEN.add(veh_id)
            _TARGET_TURN_MONITOR_COUNTS[approach]["total"] += 1
            _TARGET_TURN_MONITOR_COUNTS[approach][movement] += 1

            if wrong_lane:
                _TARGET_TURN_MONITOR_WRONG_LANE[approach]["total"] += 1

        _tm_print_turn_monitor(force=False)

    except Exception as exc:
        print(f"[target-turn-monitor] WARNING: monitor failed: {exc}", flush=True)
