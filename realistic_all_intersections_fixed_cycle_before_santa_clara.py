...
NET_FILE = os.path.join(BASE_DIR, "barcelona.net.xml")
...
OD_BALANCE_CLEANUP_INTERVAL = 5.0
...
def run_simulation_steps(
    ...
    for veh_id in active_vehicle_ids:
        ...
        # Final per-approach S/R/L balancing.  This now runs in OD mode too.
        # Endpoint balancing alone cannot prevent a shortest-path bottleneck
        # from making one intersection approach almost all-left.  This local
        # approach decision is what actually guarantees a mix whenever legal
        # alternatives exist.
    ...
    for _ in range(num_steps):
        ...
        # Expensive OD-balance bookkeeping does not need to run every SUMO step.
        # Running it every few seconds keeps counts accurate enough while avoiding
        # hundreds of extra TraCI route/vehicle calls per GUI second.
        ...
        # Blue/red spawn/destination markers are useful for diagnosis, but 1000+
        # filled polygons make SUMO-GUI very slow. They are now off by default;
        # set SPAWN_DESTINATION_MARKERS_ENABLED=True only for short visual checks.
