...
BACKGROUND_ROUTE_FILE = os.path.join(BASE_DIR, "background_santaclara.rou.xml")
AMBULANCE_ROUTE_FILE = os.path.join(BASE_DIR, "empty_ambulance_santaclara.rou.xml")
RANDOM_TRIPS_SCRIPT = os.path.join(os.environ["SUMO_HOME"], "tools", "randomTrips.py")

SUMO_RUN_LOG = os.path.join(BASE_DIR, "sumo_santaclara_proxy_run.log")
SUMO_ERROR_LOG = os.path.join(BASE_DIR, "sumo_santaclara_proxy_error.log")

TARGET_TLS_ID = "cluster_282813104_282813137_5041442783_5041442784"
REQUIRE_ALL_FOUR_PHASES_FOR_TARGET = False

# New default model name so you do not accidentally resume from the old unsafe model.
MODEL_FILE = os.path.join(BASE_DIR, "models/traffic_signal_maskable_ppo_santaclara_proxy")
...
# Keep off for speed. Turn on only when debugging SUMO crashes.
...
# Important:
# Keep this at 1800 so training statistics stay comparable to your better run.
TRAIN_EPISODE_SECONDS = 1200

MAX_NUM_VEHICLES = 2000
MAX_DEPART_DELAY = 45
TIME_TO_TELEPORT = 240
...
def discover_background_route_variants():
    variants = sorted(Path(BASE_DIR).glob("background_santaclara_train_*.rou.xml"))
    ...
    for period in periods:
        ...
        for seed in route_seeds:
            output_file = os.path.join(
                BASE_DIR,
                f"background_santaclara_train_{period_name}_seed{seed}.rou.xml",
            )
...
def build_lane_caches(movement_map):
    ...
    for label in MOVEMENT_LABELS:
        ...
        movement_in_lanes_cache[label] = lanes
    ...
    return movement_in_lanes_cache, all_in_lanes
...
def build_controller_for_tls(tls_id, activate=True):
    ...
    movement_in_lanes_cache, all_in_lanes = build_lane_caches(movement_map)
...
def train_model(
    ...
    continuing_existing_model = resume and os.path.exists(model_zip_path)

    if continuing_existing_model:
        print(f"\nLoading existing model and continuing training: {model_zip_path}")
        model = MaskablePPO.load(model_path, env=env)
    else:
        if resume:
            print("\nNo existing model found. Creating new model.")
        else:
            print("\nFresh training requested. Creating new model.")
    ...
    model.learn(
        ...
        progress_bar=True,
        reset_num_timesteps=not continuing_existing_model,
    )
...
def main():
    ...
    parser.add_argument(
        "--fresh",
        action="store_true",
        help="Start a new model even if a .zip model file already exists.",
    )
