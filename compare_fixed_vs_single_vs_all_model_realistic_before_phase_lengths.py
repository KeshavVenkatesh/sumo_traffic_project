...
def run_all_model_episode(args: argparse.Namespace, scenario: TrafficScenario, seed: int) -> dict[str, float | int | str]:
    ...
    Fast anchorless version: no target TLS, fewer TraCI metric scans, and each
    signal is updated once per --model-update-period instead of forcing heavy
    observation work every few GUI frames.
    """
    ...
    try:
        ...
        metrics_period_steps = runtime_interval_steps(args, 'metrics_interval', 10.0)
        decision_steps = 1
        ...
        print(
            ...
            f'(policy period={policy_period_steps} sim steps, metrics every {metrics_period_steps} sim steps)'
        )

        last_global_wait = 0.0
        last_global_queue = 0.0
        last_avg_speed = 0.0
        last_info: dict[str, Any] = episode.info(
            spawned=0,
            extended=0,
            recovered=0,
            arrived=0,
        ...
        for step in range(1, args.eval_steps + 1):
            ...
            do_metrics = (
                ...
                or step % args.eval_print_every == 0
            )
            if do_metrics:
                ...
                collect_sample(samples, last_info, reward=0.0)
            else:
                # Cheap info: no all-vehicle speed/wait scan.
                last_info = episode.info(
                    spawned=spawned,
                    extended=extended,
                    recovered=recovered,
            ...
            if float(last_info.get('sim_time', 0.0)) >= float(args.episode_seconds):
                break
    finally:
        episode.close()
        norm_env.close()
...
def run_fixed_cycle_episode(args: argparse.Namespace, scenario: TrafficScenario, seed: int) -> dict[str, float | int | str]:
    ...
    try:
        ...
        print(f'[fixed seed {seed}] FAST anchorless fixed-cycle control of {sum(1 for c in episode.controllers if not c.get("disabled"))} traffic lights')
