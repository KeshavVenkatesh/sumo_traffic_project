...
from fixed_demand import FIXED_DEMAND_VTYPE_ID
...
class PersistentAllTLSEpisode:
    def __init__(self, config: dict[str, Any]):
        ...
        step_length = float(self.sim.STEP_LENGTH)
        decision_seconds = float(config["decision_seconds"])
        decision_steps = decision_seconds / step_length
        if abs(decision_steps - round(decision_steps)) > 1e-9:
            raise ValueError(
                "decision_seconds must be an exact multiple of SUMO's "
                f"step length ({decision_seconds:g} / {step_length:g})"
            )
        episode_decisions = (
            float(config["episode_seconds"]) / decision_seconds
        )
        if abs(episode_decisions - round(episode_decisions)) > 1e-9:
            raise ValueError(
                "episode_seconds must be an exact multiple of "
                "decision_seconds"
            )
    ...
    def _scenario(self):
        ...
        compute_cap = int(self.config["max_vehicle_center"])
        if self.demand_routes:
            # An immutable route file already defines demand. A random density
            # draw must not silently lower SUMO's active-vehicle ceiling and
            # truncate the heavy scenarios.
            maximum = compute_cap
            target = compute_cap
            initial = 0
        else:
            density = self.py_rng.uniform(
            ...
            initial = min(
                target, max(10, int(round(target * 0.25)))
            )
        ...
        return replace(
            ...
            time_to_teleport=int(
                self.config.get(
                    "time_to_teleport", base.time_to_teleport
                )
            ),
        )
    ...
    def _start_episode(self) -> None:
        ...
        outer_args = SimpleNamespace(
            ...
            sumo_error_log=str(
                self.config.get("sumo_error_log", "") or ""
            ),
        )
        if outer_args.sumo_error_log:
            error_log = Path(
                outer_args.sumo_error_log
            ).expanduser().resolve()
            error_log.parent.mkdir(parents=True, exist_ok=True)
            outer_args.sumo_error_log = str(error_log)
        ...
        # For the final ambulance benchmark, a Native SUMO arm must leave the
        # original <tlLogic> completely untouched.  AnchorlessSimulationEpisode
        # normally builds/activates custom controllers during reset(), so
        # temporarily replace that builder with an empty one.  Read-only phase
        # metadata is reconstructed below after SUMO has started.
        native_sumo_signals = bool(
            self.config.get("native_sumo_signals", False)
        )
        if native_sumo_signals:
            original_builder = self.sim.build_all_fixed_controllers
            self.sim.build_all_fixed_controllers = (
                lambda rng, args: ([], [])
            )
            try:
                self.episode.reset()
            finally:
                self.sim.build_all_fixed_controllers = original_builder
        else:
            self.episode.reset()
        ...
        if route_file:
            ...
            if "global_car" not in type_ids:
                source_type = (
                    ...
                    if FIXED_DEMAND_VTYPE_ID in type_ids
                    else next(
                        (
                            type_id
                ...
                if source_type is None:
                    source_type = next(
                        (
                            ...
                            for type_id in type_ids
                            if str(
                                self.sim.traci.vehicletype.getVehicleClass(
                                    type_id
                                )
                            )
                            == "passenger"
        ...
        self.cache = MapTrafficSnapshot(
            self.sim.traci,
            self.sim,
            exclude_vehicle=(
                lambda vehicle_id: str(vehicle_id).startswith(
                    excluded_prefixes
                )
            )
        ...
        adapter_controllers = self.episode.controllers
        if native_sumo_signals:
            # These controller dictionaries exist only so MapAgnosticTLSAdapter
            # can map route transitions/link indices to native stable greens.
            # activate=False is essential: it makes no TraCI signal command.
            metadata_rng = random.Random(
                int(scenario.seed) ^ 0x5A17C0DE
            )
            adapter_controllers = []
            for tls_id in sorted(
                self.sim.traci.trafficlight.getIDList()
            ):
                controller = (
                    self.sim.build_map_agnostic_controller_for_tls(
                        tls_id,
                        rng=metadata_rng,
                        activate=False,
                    )
                ...
                if controller is not None:
                    adapter_controllers.append(controller)
        ...
        if bool(self.config.get("strict_exit_space", False)):
            required_gap = float(
                self.config.get("required_exit_gap_meters", 18.0)
            )
            for adapter in self.adapters:
                adapter.controller["green_activation_guard"] = (
                    lambda phase_position, adapter=adapter, gap=required_gap: (
    ...
    def _action_mask(self, adapter) -> np.ndarray:
        return adapter.action_mask(
            ...
            require_exit_space=bool(
                self.config.get("strict_exit_space", False)
            ),
            required_exit_gap_meters=float(
                self.config.get("required_exit_gap_meters", 18.0)
            ),
            allow_unsafe_hard_max_fallback=bool(
                self.config.get(
    ...
    def _apply_action(self, adapter, action: int) -> tuple[bool, bool]:
        ...
        if action < 0 or action >= len(mask) or not mask[action]:
            controller["invalid_policy_actions"] = int(
                controller.get("invalid_policy_actions", 0)
            ) + 1
    ...
    def step(self, actions: np.ndarray, reset_on_done: bool = True):
        ...
        if bool(self.config.get("native_sumo_signals", False)):
            # episode.controllers is empty in this mode, so SUMO advances the
            # map's own traffic-light program.  Never call _apply_action(),
            # which would invoke setRedYellowGreenState/request_switch.
            switches = [(False, False)] * len(self.adapters)
        else:
            switches = [
