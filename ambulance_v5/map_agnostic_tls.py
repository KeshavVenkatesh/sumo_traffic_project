...
DEFAULT_REQUIRED_EXIT_GAP_METERS = 18.0
...
class MapTrafficSnapshot:
    ...
    def __init__(
        ...
        exclude_vehicle: Callable[[str], bool] | None = None,
    ):
        self.traci = traci_module
        self.sim = simulation_module
        self.exclude_vehicle = exclude_vehicle
    ...
    def refresh(self, adapters: Sequence["MapAgnosticTLSAdapter"]) -> None:
        ...
        for lane_id in all_lanes:
            ...
            self.lane_vehicle_ids[lane_id] = tuple(
                ...
                if self.exclude_vehicle is None
                or not self.exclude_vehicle(str(vehicle_id))
            )
...
class MapAgnosticTLSAdapter:
    ...
    def action_mask(
        ...
        require_exit_space: bool = False,
        required_exit_gap_meters: float = (
            DEFAULT_REQUIRED_EXIT_GAP_METERS
        ),
        allow_unsafe_hard_max_fallback: bool = True,
    ) -> np.ndarray:
        ...
        if (
            allow_unsafe_hard_max_fallback
    ...
    def _lane_has_exit_gap(
        self, lane_id: str, required_gap_meters: float
    ) -> bool:
        """Fail closed unless the lane entrance has the requested clear gap."""

        try:
            vehicle_ids = tuple(
                self.traci.lane.getLastStepVehicleIDs(lane_id)
            )
            if not vehicle_ids:
                return True
            closest_position = min(
                float(self.traci.vehicle.getLanePosition(vehicle_id))
                for vehicle_id in vehicle_ids
            )
            return closest_position + 1e-9 >= max(
                0.0, float(required_gap_meters)
            )
        except Exception:
            return False

    def _phase_has_exit_space(
        self,
        candidate: int,
        required_gap_meters: float,
        lane_space_cache: dict[str, bool] | None = None,
    ) -> bool:
        if candidate < 0 or candidate >= self.phase_count:
            return False
        movement_indices = self.topology.phase_members[candidate]
        cache = lane_space_cache if lane_space_cache is not None else {}
        for movement_index in movement_indices:
            for lane_id in self.topology.movements[
                movement_index
            ].outgoing_lanes:
                if lane_id not in cache:
                    cache[lane_id] = self._lane_has_exit_gap(
                        lane_id, required_gap_meters
                    )
                if not cache[lane_id]:
                    return False
        return True

    def phase_position_has_exit_space(
        self, phase_position: int, required_gap_meters: float
    ) -> bool:
        """Check a native SUMO phase position immediately before activation."""

        try:
            candidate = self.topology.phase_positions.index(
                int(phase_position)
            )
        except ValueError:
            return False
        return self._phase_has_exit_space(
            candidate, required_gap_meters
        )
