...
def load_base_policy(path: str | Path):
    """Load shared schema-v3 scorer weights without rebuilding its old spaces.

    SB3 serializes the original padded observation/action spaces inside the
    archive. Rebuilding the full policy would therefore reject a runtime with
    a larger MAX_PHASES even though MovementGraphNetwork has no phase-position-
    specific parameters. Loading only map_network's state dict preserves the
    frozen weights exactly while allowing additional padded candidates.
    """
    ...
    if not isinstance(policy_state, dict):
        raise RuntimeError(
            f"Base checkpoint has no policy state dict: {checkpoint}"
        )
    ...
    if not network_state:
        raise RuntimeError(
            f"Base checkpoint has no {prefix} weights: {checkpoint}"
        )

    action_space = data.get("action_space") if isinstance(data, dict) else None
    source_actions = int(getattr(action_space, "n", 0))
    source_max_phases = source_actions - 1
    if source_max_phases <= 0:
        raise RuntimeError(
            f"Cannot determine source phase capacity from {checkpoint}"
        )
    if source_max_phases != 16:
        raise RuntimeError(
            "This audited padding expansion expects the frozen schema-v3 "
            f"checkpoint to have 16 phases, found {source_max_phases}"
        )
    if source_max_phases > MAX_PHASES:
        raise RuntimeError(
            "Base checkpoint phase capacity exceeds runtime: "
            f"checkpoint={source_max_phases}, runtime={MAX_PHASES}"
        )

    first_weight = network_state.get("movement_encoder.0.weight")
    if first_weight is None or first_weight.ndim != 2:
        raise RuntimeError(
            f"Base checkpoint movement encoder is missing: {checkpoint}"
        )
    embed_dim = int(first_weight.shape[0])
    graph_indices = {
        int(parts[1])
        for key in network_state
        for parts in (key.split("."),)
        if len(parts) > 2
        and parts[0] == "graph_blocks"
        and parts[1].isdigit()
    }
    graph_layers = max(graph_indices) + 1 if graph_indices else 0
    ...
    if source_max_phases != MAX_PHASES:
        print(
            "[base loader] expanded shared phase scorer padding: "
            f"{source_max_phases} -> {MAX_PHASES}; frozen weights unchanged",
            flush=True,
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--validation-interval-rounds",
        type=int,
        default=1,
        help="Run validation every N rounds and after the final round",
    )
    parser.add_argument(
    ...
    if args.validation_interval_rounds <= 0:
        parser.error(
            "--validation-interval-rounds must be positive"
        )
...
def main() -> None:
    ...
    ambulance_system_contract = {
        ...
        "base_phase_padding": {
            "source_max_phases": 16,
            "runtime_max_phases": MAX_PHASES,
            "expansion": "shared_phase_scorer_state_dict",
            "frozen_weights_changed": False,
        },
        "planned_active_duration_factor": (
            args.planned_active_duration_factor
        ),
    ...
    if args.validate_every_round:
        print(
            "  validation schedule:     every "
            f"{args.validation_interval_rounds} round(s), plus final"
        )
    else:
        print("  validation schedule:     disabled")
    ...
    try:
        for round_index in range(args.rounds):
            ...
            validation_due = (
                round_number % args.validation_interval_rounds == 0
                or round_number == args.rounds
            )
            if (
                ...
                write_progress(
                    ...
                    "validating",
                    {
                        ...
                        "validation_interval_rounds": (
                            args.validation_interval_rounds
                        ),
                        "validation_episode_seconds": (
                            args.validation_episode_seconds
                        ),
                        "validation_seeds": parse_csv(
                            args.validation_seeds
                        ),
                    },
                )
                ...
                trainer.last_validated_round = round_number
                trainer.save(total_updates)
        trainer.save(total_updates)
        write_progress(
