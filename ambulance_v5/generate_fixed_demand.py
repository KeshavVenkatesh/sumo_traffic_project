...
def generate_one(
    ...
    if output.exists() and not force:
        ...
        if not fixed_demand_vehicle_type_is_safe(output):
            raise RuntimeError(
                "Existing demand route does not use the audited passenger "
                "vehicle type with jmIgnoreKeepClearTime=-1: "
                f"{output}. Regenerate with --force."
            )
        return {
            "seed": seed,
    ...
    enforce_fixed_demand_vehicle_type(output)
    scheduled_records = count_routes(output)
    if scheduled_records <= 0:
        raise RuntimeError(
