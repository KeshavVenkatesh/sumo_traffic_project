...
def set_route_lane_commitment_lock(veh_id, current_edge):
    """Mark the vehicle as route-lane committed without hard-freezing recovery.

    The earlier strict patch used laneChangeMode=0 here.  That made vehicles
    look disciplined, but it also boxed them in when the target lane was blocked
    or when SUMO could not complete the requested merge.  Keep normal SUMO lane
    changing available; the route-lane preference helper will keep nudging the
    car back toward a route-compatible lane when needed.
    """
    try:
        traci.vehicle.setLaneChangeMode(veh_id, TRAFFIC_LIGHT_NORMAL_LANE_CHANGE_MODE)
...
def apply_traffic_light_lane_change_lock_to_vehicle(veh_id):
    """Disable late autonomous lane changes only when the current lane is valid.

    If the vehicle is already in a lane that cannot reach its planned next edge,
    do not hard-lock it.  Leave lane changing enabled so the rescue/reroute
    helpers can either move it to the correct lane or rewrite the route to a
    reachable outgoing edge.
    """
    try:
    ...
    next_edge = planned_next_edge_from_route(veh_id, current_edge) if current_edge is not None else None
    lane_matches_route = (
        next_edge is None
        or next_edge.startswith(":")
        or lane_has_connection_to_edge(lane_id, next_edge)
    )

    if inside_traffic_light_no_lane_change_zone(lane_id, distance_to_end) and lane_matches_route:
        try:
    ...
    if veh_id in TRAFFIC_LIGHT_LANE_CHANGE_LOCKED_VEHICLES or not lane_matches_route:
        try:
            traci.vehicle.setLaneChangeMode(veh_id, TRAFFIC_LIGHT_NORMAL_LANE_CHANGE_MODE)
        except traci.TraCIException:
            pass
...
def force_route_to_reachable_lane_successor(
    ...
    Prefer a legal straight/right movement when possible, preserve the old OD
    destination when possible, and always release stale speed/lane locks.
    """
    outgoing_edges = lane_outgoing_edges(current_lane, allowed_edges=raw_graph)
    if not outgoing_edges:
        outgoing_edges = lane_outgoing_edges(current_lane)
    if not outgoing_edges:
        return False

    try:
        blocked_next_edge = planned_next_edge_from_route(veh_id, current_edge)
    except Exception:
        blocked_next_edge = None
    ...
    if reachable_edge is None:
        ...
        for edge in candidates:
            movement = movement_for_lane_to_edge(current_lane, edge)
            movement_weight = TURN_PROBABILITIES.get(movement, 0.05)
            if movement == "S":
                movement_weight *= 4.0
            elif movement == "R":
                movement_weight *= 1.8
            ...
            weights.append(
                successor_weight(
                    ...
                    args=args,
                ) * movement_weight * loop_avoidance_weight_multiplier(current_edge, edge, recent_edges_set)
            )
        reachable_edge = weighted_choice(rng, candidates, weights)

    destination = destination_from_current_route(veh_id, current_edge)
    route = build_destination_preserving_route(current_edge, reachable_edge, destination)
    return set_vehicle_route_and_release_controls(veh_id, route)
