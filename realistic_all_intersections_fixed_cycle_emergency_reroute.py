...
# Emergency fallback for strict route-lane commitment.
# Vehicles still try to reach the correct route-compatible lane early. If a car
# is boxed in and its current lane cannot legally reach the next routed edge, it
# should not block the signal forever. In that case, rewrite the route to a
# reachable outgoing edge from the current lane.
EMERGENCY_ROUTE_REROUTE_DISTANCE = 140.0
EMERGENCY_ROUTE_REROUTE_STOPPED_TIME = 4.0
EMERGENCY_ROUTE_REROUTE_SPEED = 0.35
EMERGENCY_ROUTE_REROUTE_COOLDOWN = 12.0
...
def set_route_lane_commitment_lock(veh_id, current_edge):
    ...
    The old strict patch used lane-change mode 0 here, which could trap cars
    behind a signal if they later needed an emergency recovery maneuver. Keep
    normal SUMO lane changing enabled; the preference helper will continue
    nudging the car back toward route-compatible lanes, while the hard
    no-lane-change zone still prevents last-second weaving near the stop line.
    """
    try:
        traci.vehicle.setLaneChangeMode(veh_id, TRAFFIC_LIGHT_NORMAL_LANE_CHANGE_MODE)
...
def movement_for_lane_successor(current_lane, outgoing_edge):
    """Classify the movement from current_lane to outgoing_edge, if SUMO exposes it."""
    for link in get_lane_links(current_lane):
        if not link:
            continue
...
def candidate_emergency_outgoing_edges(current_lane, current_edge, preferred_movement=None, previous_edge=None):
    """Reachable outgoing edges from the current lane, sorted by how reasonable they are."""
    candidates = []
    for outgoing_edge in lane_outgoing_edges(current_lane):
        if not outgoing_edge:
            continue
        if outgoing_edge == current_edge:
            continue
        if outgoing_edge in HARDCODED_NO_CRUISE_LOOP_EDGES:
            continue

        movement = movement_for_lane_successor(current_lane, outgoing_edge)
        same_movement = 0 if movement == preferred_movement else 1
        backtrack = 1 if previous_edge is not None and outgoing_edge == previous_edge else 0
        candidates.append((same_movement, backtrack, outgoing_edge))

    candidates.sort()
    return [edge for _same, _back, edge in candidates]
...
def emergency_reroute_vehicle_from_blocked_lane(
    ...
    """Reroute a boxed-in car so it clears the intersection instead of clogging it.

    This is only for the bad case where the current lane cannot legally reach
    the next edge in the vehicle's route and it is too late or too blocked to
    change into the correct lane. We first try to preserve the original final
    destination by routing from a reachable outgoing edge to the old destination.
    If that fails, we use a short legal route just to clear the current junction.
    """
    if is_ambulance(veh_id):
        return False
    ...
    previous_edge = old_route[route_index - 1] if 0 < route_index < len(old_route) else None
    destination = old_route[-1] if old_route else None

    outgoing_edges = candidate_emergency_outgoing_edges(
        current_lane=current_lane,
        current_edge=current_edge,
    ...
    for outgoing_edge in outgoing_edges:
        # Best case: preserve the original destination via SUMO fastest routing.
        if destination and destination != current_edge and destination != outgoing_edge:
            try:
                path = fastest_sumo_route(outgoing_edge, destination)
                edges = list(getattr(path, "edges", []) or [])
            except traci.TraCIException:
                edges = []

            if edges and edges[0] == outgoing_edge and len(edges) >= 1:
                route_candidates.append([current_edge] + edges)

        # Fallback: at least clear the current junction legally.
        route_candidates.append([current_edge, outgoing_edge])
    ...
    for route in route_candidates:
        ...
        for edge_id in route:
            if not edge_id or edge_id.startswith(":"):
                continue
            if edge_id in HARDCODED_NO_CRUISE_LOOP_EDGES and edge_id != current_edge:
                continue
            if cleaned and cleaned[-1] == edge_id:
                continue
            cleaned.append(edge_id)
...
def should_emergency_reroute_blocked_lane(
    veh_id,
    lane_id,
    distance_to_end,
    speed,
    ...
    """Return True when a vehicle has genuinely failed to reach the needed lane."""
    if lane_has_connection_to_edge(lane_id, next_edge):
        return False

    if distance_to_end <= EMERGENCY_ROUTE_REROUTE_DISTANCE:
        return True
...
def emergency_reroute_if_needed_for_lane_preference(
    veh_id,
    current_edge,
    lane_id,
    next_edge,
    ...
    if not should_emergency_reroute_blocked_lane(
        veh_id=veh_id,
        lane_id=lane_id,
        distance_to_end=distance_to_end,
        speed=speed,
...
def apply_turn_lane_preference_to_vehicle(veh_id):
    ...
    if no_change_buffer > 0.0 and distance_to_end <= no_change_buffer + required_distance:
        return emergency_reroute_if_needed_for_lane_preference(
            veh_id=veh_id,
            current_edge=current_edge,
            lane_id=lane_id,
            next_edge=next_edge,
    ...
    if speed < LANE_PREF_MIN_SPEED or waiting_time > LANE_PREF_MAX_WAITING_TIME:
        return emergency_reroute_if_needed_for_lane_preference(
            veh_id=veh_id,
            current_edge=current_edge,
            lane_id=lane_id,
