...
def run_all_model_episode(args: argparse.Namespace, scenario: TrafficScenario, seed: int) -> dict[str, float | int | str]:
    ...
    try:
        ...
        # GUI smoothness fix:
        # The old all-model loop updated every controller, predicted a full
        # model-action batch, computed stats, and then advanced SUMO by a
        # 5-second block.  In SUMO-GUI that shows up as a visible hitch at
        # t=4,9,14,19,... because the expensive Python/model work happens at
        # the end of each 5-step block.
        #
        # Instead, advance SUMO one second at a time and stagger controller
        # policy updates across the same 5-second decision window.  Each TLS is
        # still updated roughly once per DECISION_INTERVAL, but the work is
        # spread across frames instead of happening in one big lump.
        decision_period_steps = max(1, int(round(sim.DECISION_INTERVAL / sim.STEP_LENGTH)))
        decision_steps = 1

        controllable_count = sum(1 for c in env.controllers if not c.get('disabled'))
        print(
            f'[all-model seed {seed}] controlling {controllable_count} traffic lights with copied policy '
            f'(smooth GUI: staggered over {decision_period_steps} sim steps)'
        )

        for step in range(1, args.eval_steps + 1):
            active_controllers = [c for c in env.controllers if not c.get('disabled')]
            if active_controllers:
                bucket = (step - 1) % decision_period_steps
                controllers_to_update = [
                    controller
                    for idx, controller in enumerate(active_controllers)
                    if idx % decision_period_steps == bucket
                ]
